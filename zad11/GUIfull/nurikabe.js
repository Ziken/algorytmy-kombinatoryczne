...
var MakeMove = function(board, islands, moves, move) {
  ...
  moves.push(move);
    $("td#"+ move.field.x + "x" + move.field.y)
    .removeClass("black")
    .addClass("white");
...
var MoveForward = function(board, islands, moves, t) {
...
var Nurikabe = function (n, m, t, islands) {
  ...
  for(i = 0; i < m; i++){
    ...
    for(j = 0; j < n; j++){
      $("#board tr#"+ i).append("<td id="+ i + "x" + j +" class=\"black\"></td>");
      Board[i][j] = { "Island": -1 };
    }
  }
  $("#board").css({"width": 46 * n});
  ...
  //solve Nurikabe
  return MoveForward( Board, Islands, Moves, t );
};
